...
# #     """Generate a response to a user question based on the audio context using Gemini."""
...
# #         "Provide accurate and concise answers relevant to the provided context. If the question is unrelated to the context, "
# #         "politely inform the user that the question is outside the scope of the provided audio content.\n\n"
...
# # -------------------------------
# # HELPERS FOR TRENDY TIMESTAMPS & CLIPS (FINAL)
# # - Now supports: lowered threshold, timestamp fallback from text, and strict/loose modes
# # -------------------------------
...
#     """
#     # common patterns: hh:mm:ss or mm:ss
...
#       1) Exact/loose transcript matching (sliding windows)
#       2) If fails and allow_bracket_fallback==True, parse bracketed timestamps in the item_text
#       3) As a last resort (only in 'loose' mode) return the best single utterance expanded to min length
#     Parameters:
#       mode: 'strict' or 'loose' — strict uses higher score cutoff and avoids bracket fallback by default
#       allow_bracket_fallback: if True, will try to parse timestamps like [A 13:55 - 14:25]
#     """
#     if not diarization_data or not item_text or not item_text.strip():
#         return None

#     # 0. Try to parse explicit bracket timestamps first (if allowed) — this is safest when present
#     if allow_bracket_fallback:
#         br = _parse_bracket_timestamp(item_text)
#         if br:
#             return br

#     # 1. Sliding-window transcript matching
...
#     # 2. If no good sliding-window match, try fragments (split by punctuation)
#     fragments = [f.strip() for f in re.split(r"[.,;:!?]\s*", item_text) if f.strip()]
#     for frag in fragments:
#         frag_norm = _normalize_text(frag)
#         if not frag_norm:
#             continue
#         # search over utterances (single best or small windows)
#         best_frag = None
#         for i in range(n):
#             if not norm_texts[i]:
#                 continue
...
#                 if seg_norm:
#                     concat.append(seg_norm)
#                 end_time = diarization_data[j].get("end", end_time) or end_time
#                 window_text = " ".join(concat)
...
#     # 3. Final fallback (only in loose mode): return the single best utterance expanded slightly
#     if mode == "loose":
#         best_single = None
...
#             sc, s, e = best_single
#             # don't accept a super-low match — require small positive overlap
#             if sc >= 0.12:
#                 if e - s < MIN_CLIP_MS:
#                     e = s + MIN_CLIP_MS
#                 return int(s), int(e)
...
# # small UI control: mode and fallback option
# with st.sidebar:
...
# # (rest of UI remains same as before; integrate updated find_trendy_span_ms call with mode and allow_bracket)
...
# # Main app area (kept similar to previous structure)
...
# def save_to_db(file_name, summary, dialogue, diarization):
#     conn = sqlite3.connect("aura_vox.db")
#     cursor = conn.cursor()
#     cursor.execute("""
#         INSERT INTO transcripts (file_name, summary, dialogue, diarization)
#         VALUES (?, ?, ?, ?)
#     """, (file_name, summary, dialogue, str(diarization)))
#     conn.commit()
#     conn.close()
...
#                 "\n".join(st.session_state['aligned_dialogue']),
#                 st.session_state['diarization_data']
#                 )
    ...
    try:
        ...
        if trendy_clips:
            ...
            for clip_text, start_ms, end_ms, clip_bytes in trendy_clips:
                clip_path = f"clips/transcript_{transcript_id}_clip_{start_ms}-{end_ms}.mp3"
